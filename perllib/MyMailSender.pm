...
##  $Log: $
############################################################################
package perllib::MyMailSender;

=head1 NAME

B<perllib::MyMailSender> - Handle MIME emails for attachments.

 B<new()> - create an instance of this package.  Not exported.  B<USAGE:>
 my $myObject = new perllib::MyMailSender(
                          'mailhost'     => $host,
...
sub new()
{
	my perllib::MyMailSender $self	= shift;    # Me.
	...
	if( keys( %{$opts} ) )
	{
		print "perllib::MyMailSender::new(): Illegal arguments.\n";
		foreach my $key ( %{$opts} )
		{
...
sub closeMe()
{
	my perllib::MyMailSender $self = shift;
	$self = undef;
}
...
=head2
	...
	Logs the MyMailSender hash for this object.
...
sub send()
{
	...
	my $rs;
	eval{
		$rs		= $msg->Close( 1 ) or $self->logIt( ref( $self ) . "::send(): Error sending the email:$!\n" );
	};
	if( $@ )
	{
...
sub attach($$)
{
	...
	$self->{mime_container}->Attach( 
									{
										description => 'Perl module Mail::Sender.pm',
										ctype 		=> $type, 
										disposition	=> 'attachment; filename="$baseFile"',
										file 		=> $file
									} 
	) or $self->logIt( ref( $self ) . "::attach(): Error adding " . $file . ":$!\n" );
	return 0;
}
...
sub init()
{
	...
	my @to = split( /\|/, $to );
	my $to = join( ",", @to );
	$self->{mime_container} = Mail::Sender->new( { smtp	=> $self->{mailhost}, from => $self->{from_address}, debug => $self->{funcs_object}->getLogFH() } );
	$self->{mime_container}->OpenMultipart( 
											{ 
												to		=> $to,
												subject	=> $self->{subject}
											} 
	);
	$self->{mime_container}->Body( { msg				=> $self->{message} } );
	
	if( defined( $self->{attachment} ) && $self->{attachment} ne "" )
	{
...
=head1 EXAMPLE
    ...
    ##########################################
    #    Get an instance of the MyMailSender.
    ##########################################
    my $myObject = new perllib::MyMailSender(
