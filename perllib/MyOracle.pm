...
##  $Header: $
##
##  Perl module to handle Oracle connections and queries.
##  $Author: putnam $
############################################################################
##
...
B<perllib::MyOracle> - Perl module to handle Oracle database connectivity
and queries.

 B<new()> - create an instance of this package.  Not exported.  B<USAGE:>
 my $myObject = new perllib::MyOracle(
                          'sid' => "ORC",
                          'user' => "user_name",
                          'pwd' => "password",
                          'trace_level' => 9,
...
sub new()
{
	#my perllib::MyOracle $myPackage	= shift;   	# Me.
	#my( $self ) 					= {@_};   	# Stores all the keys and values passed to this function.
	my perllib::MyOracle $self		= shift;    # Me.
...
sub closeMe()
{
	my perllib::MyOracle $self = shift;
	$self->{dbh}->disconnect();
	$self = undef;
}
###############################################################################
...
B<printMainHash()> - Logs the MyOracle hash for this object.
...
None at the time of this writing.  May 06, 2011

=head1 AUTHOR

Denis M. Putnam, May 06, 2011.
